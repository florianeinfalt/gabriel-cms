    ...
    if request.method == 'POST':
        if register_form.validate_on_submit():
            ...
            new_user = User(name=register_form.name.data,
                            ...
                            password=utilities.hash_password(register_form.password.data),
                            role=user_role)
    ...
    if request.method == 'POST':
        if login_form.validate_on_submit():
            ...
            if not user.is_live:
                ...
                return redirect(url_for('login'))
            if not utilities.check_password(login_form.password.data, user.password):
...
def change_password():
    ...
    if request.method == 'POST':
        if change_password_form.validate_on_submit():
            ...
            if not utilities.check_password(old_password, User.query.filter_by(email=current_user.email).first().password):
                flash('Old password is invalid')
                return redirect(url_for('change_password'))
            if not utilities.password_is_valid(new_password):
                flash('Password is not strong enough.\nNeeds to contain lower case and upper case letters, numericals and punctuation.\nLength must be between 8 and 12 characters.')
                return redirect(url_for('change_password'))
            User.query.filter_by(email=current_user.email).first().password = utilities.hash_password(new_password)
            db.session.commit()
